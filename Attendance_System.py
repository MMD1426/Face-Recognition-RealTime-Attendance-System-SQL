...
class FunctionsQuery():
    ...
    def create_database(self):
        ...
        connect = sqlite3.connect('d:\Project-VSCode\Face-Recognition-Attendence-System-SQL\database.db')
    ...
    def insert(self):
        ...
        if self.checkOpen() and not self.checkExit():
            ...
            difference = datetime.datetime.strptime(str(self.clockExit()[0][0]),FMT) - datetime.datetime.strptime(str(self.clockOpen()[0][0]),FMT)
            difference = datetime.datetime.strptime(str(difference),FMT).hour
            if int(difference) < 12:
                self.insertOpen()
            elif int(difference) > 12:
                self.insertExit()
        else:
            self.insertOpen()
